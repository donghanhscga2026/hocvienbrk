...
const RoadmapBuilderContent = () => {
  ...
  const onDrop = useCallback(
    (event: any) => {
      ...
      const newNode = {
        ...
        type,
        position,
        data: { label: `Nội dung ${type === 'questionNode' ? 'câu hỏi' : 'mới'}...` },
      };
      ...
      if (Array.isArray(q.options)) {
        q.options.forEach((opt: any, optIndex: number) => {
          ...
          newNodes.push({ 
            id: optNodeId, 
            type: 'optionNode', 
            position: { x: x + qIndex * spacingX + (optIndex * 160 - 120), y: y + spacingY }, 
          ...
          if (opt.isAdvice) {
              ...
              newNodes.push({
                  id: adviceNodeId,
                  type: 'adviceNode',
                  position: { x: x + qIndex * spacingX + (optIndex * 160 - 120), y: y + spacingY * 2 },
                  data: { label: 'https://youtube.com' }
              });
              newEdges.push({ id: `e_${optNodeId}_${adviceNodeId}`, source: optNodeId, target: adviceNodeId });
          }
        });
      }
    });
    ...
    <div className="space-y-8 animate-in fade-in duration-500 text-black max-w-6xl mx-auto">
      <div className="flex justify-between items-end border-b border-gray-200 pb-6">
        ...
        <button 
            ...
            className="bg-black text-yellow-400 px-8 py-4 rounded-[2rem] font-black uppercase text-xs flex items-center gap-2 hover:scale-105 transition-all shadow-2xl hover:shadow-yellow-400/20 active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {surveys.length === 0 && (
            <div className="col-span-full py-20 text-center bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
                <p className="text-gray-400 font-bold uppercase text-xs">Chưa có bài khảo sát nào. Hãy tạo bài đầu tiên!</p>
            </div>
        )}
        {surveys.map((survey) => (
          <div key={survey.id} className={`bg-white rounded-[3rem] p-8 border-2 transition-all group relative overflow-hidden ${survey.isActive ? 'border-green-500 shadow-2xl shadow-green-500/10' : 'border-gray-100 hover:border-gray-300'}`}>
            {survey.isActive && (
                <div className="absolute top-0 right-0 bg-green-500 text-white px-6 py-1.5 rounded-bl-[1.5rem] text-[9px] font-black uppercase tracking-widest shadow-lg">
                    Đang chạy
                </div>
            )}
            ...
            <div className="grid grid-cols-1 gap-3">
              <button 
                ...
                className="flex items-center justify-center gap-3 bg-zinc-900 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-black transition-all active:scale-95 shadow-lg shadow-black/10"
              >
              ...
              <div className="grid grid-cols-2 gap-3">
                  {!survey.isActive ? (
                    <button 
                        ...
                        className="flex items-center justify-center gap-2 bg-green-50 text-green-600 py-4 rounded-2xl font-black uppercase text-[9px] tracking-tighter hover:bg-green-500 hover:text-white transition-all active:scale-95"
                    >
                  ...
                  <button 
                    ...
                    className="flex items-center justify-center gap-2 bg-red-50 text-red-500 py-4 rounded-2xl font-black uppercase text-[9px] tracking-tighter hover:bg-red-500 hover:text-white transition-all active:scale-95"
                  >
  ...
  // VIEW: TRÌNH THIẾT KẾ SƠ ĐỒ (EDITOR)
  return (
    <div className="h-[calc(100vh-150px)] flex flex-col bg-white rounded-[3rem] overflow-hidden shadow-2xl border border-gray-100 animate-in slide-in-from-bottom-4 duration-500 text-black relative">
      ...
      <div className="bg-black p-5 flex justify-between items-center z-50">
        <div className="flex items-center gap-6">
            <button 
                ...
                className="p-3 bg-white/10 text-white rounded-2xl hover:bg-white/20 transition-all active:scale-90"
            >
                <ArrowLeft className="w-5 h-5" />
            </button>
            <div>
                <h2 className="text-white font-black uppercase tracking-tighter text-sm italic flex items-center gap-2">
                    <span className="text-yellow-400">Thiết kế:</span> {surveys.find(s => s.id === currentSurveyId)?.name}
                </h2>
            ...
            <button 
                ...
                className="bg-white/10 text-white border border-white/20 px-4 py-2 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-white/20 transition-all flex items-center gap-2"
            >
                <RefreshCw className="w-3 h-3" /> Nạp từ bản cũ
            </button>
        </div>
        <div className="flex items-center gap-4">
            ...
            <button 
                onClick={onSave} 
                disabled={isSaving} 
                className="bg-yellow-400 text-black px-10 py-3 rounded-2xl font-black uppercase text-[11px] hover:scale-105 transition-all disabled:opacity-50 flex items-center gap-2 shadow-xl shadow-yellow-400/20 active:scale-95"
            >
      ...
      <div className="flex flex-1 overflow-hidden" ref={reactFlowWrapper}>
        ...
        <aside className="w-72 bg-gray-50 border-r border-gray-200 p-6 flex flex-col gap-4 overflow-y-auto no-scrollbar">
          <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 border-b border-gray-200 pb-2 flex items-center gap-2">
            <Plus className="w-3 h-3" /> Thư viện khối kiến trúc
          </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            {[
              ...
              <div 
                key={tool.type} 
                className={`p-4 bg-white border-2 border-gray-100 rounded-2xl cursor-grab active:cursor-grabbing hover:border-${tool.color === 'zinc' ? 'gray' : tool.color}-500 transition-all group shadow-sm`} 
                onDragStart={(e) => e.dataTransfer.setData('application/reactflow', tool.type)} 
                draggable
              >
                <div className={`text-${tool.color === 'zinc' ? 'gray' : tool.color}-600 text-[11px] font-black uppercase group-hover:scale-105 transition-transform`}>
                    {tool.label}
                </div>
          ...
          {/* Properties Panel (Hiện ra khi click vào Node) */}
          {selectedNode && (
            <div className="mt-8 pt-8 border-t-2 border-dashed border-gray-200 space-y-5 animate-in fade-in slide-in-from-top-4 duration-300">
              <div className="flex justify-between items-center">
                <div className="text-[10px] font-black text-black uppercase tracking-widest bg-yellow-400 px-3 py-1 rounded-lg shadow-sm">
                    Cấu hình khối
                </div>
              ...
              <div className="space-y-4 bg-white p-5 rounded-[2rem] border-2 border-gray-100 shadow-sm">
                  <div className="space-y-1.5">
                    <label className="text-[9px] font-black uppercase text-gray-400 ml-1 italic">Nội dung hiển thị</label>
                    <textarea 
                        className="w-full p-4 text-xs font-bold border-2 border-gray-50 rounded-2xl outline-none focus:ring-4 ring-yellow-400/20 focus:border-yellow-400 transition-all text-black" 
                  ...
                  {selectedNode.type === 'questionNode' && (
                    <div className="space-y-1.5">
                        ...
                        <select 
                            className="w-full p-4 text-xs font-bold border-2 border-gray-50 rounded-2xl outline-none focus:border-orange-500 transition-all text-black bg-white"
                  ...
                  {selectedNode.type === 'courseNode' && (
                    <div className="space-y-1.5">
                        <label className="text-[9px] font-black uppercase text-gray-400 ml-1 italic">Chọn khóa học từ hệ thống</label>
                        <select 
                            className="w-full p-4 text-xs font-bold border-2 border-gray-50 rounded-2xl outline-none focus:border-purple-500 transition-all text-black bg-white" 
                  ...
                  <button 
                    onClick={() => { setNodes((nds: any) => nds.filter((n: any) => n.id !== selectedNode.id)); setSelectedNode(null); }} 
                    className="w-full py-4 bg-red-50 text-red-600 text-[10px] font-black uppercase rounded-2xl border-2 border-red-100 hover:bg-red-500 hover:text-white hover:border-red-500 transition-all active:scale-95"
                  >
                    Xóa khối này
                  </button>
              </div>
          ...
          >
            ...
            <MiniMap 
                nodeStrokeColor={(n: any) => {
                    if (n.type === 'questionNode') return '#f97316';
                    if (n.type === 'courseNode') return '#9333ea';
                    if (n.type === 'adviceNode') return '#3b82f6';
                    return '#9ca3af';
                }}
                nodeColor={(n: any) => {
                    if (n.type === 'questionNode') return '#fff7ed';
                    if (n.type === 'courseNode') return '#faf5ff';
                    if (n.type === 'adviceNode') return '#eff6ff';
                    return '#f9fafb';
                }}
                className="!bg-white !rounded-[2rem] !border-2 !border-gray-100 !shadow-2xl"
            />
            <Panel position="top-right" className="bg-black/80 p-4 rounded-2xl text-[9px] font-black uppercase text-yellow-400 border border-white/10 shadow-2xl backdrop-blur-xl tracking-widest italic">
              <p>💡 Click: Sửa • Kéo: Nối • Del: Xóa dây</p>
            </Panel>
          </ReactFlow>
        </div>
      </div>
