...
import {
  saveSurveyFlow, getCoursesForBuilder, getSurveyFlow
} from '@/app/actions/roadmap-actions';
import { surveyQuestions } from '@/lib/survey-data';
import { Loader2, ArrowLeft, Trash2, Save, RefreshCw } from 'lucide-react';
...
const RoadmapEditorContent = ({ surveyId, surveyName, onBack, onSaved }: RoadmapEditorProps) => {
  ...
  const loadFlow = async () => {
    ...
    try {
      const flowData = await getSurveyFlow(surveyId);
  ...
  const onSave = async () => {
    setIsSaving(true);
    const result = await saveSurveyFlow(surveyId, { nodes, edges });
    if (result.success) {
    ...
    <div className="fixed inset-0 z-[100] md:relative md:h-[calc(100vh-150px)] flex flex-col bg-[#F8F9FA] overflow-hidden text-black">
      <div className="bg-black p-3 md:p-4 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
            <button onClick={onBack} className="p-2 bg-white/10 text-white rounded-xl"><ArrowLeft className="w-4 h-4" /></button>
