...
### Mục tiêu
Giải quyết triệt thế lỗi không tải được ảnh avatar từ các domain ngoài do chính sách CORS của trình duyệt chặn.
### Các file đã tạo/sửa
...
## ✅ PHẦN 27: SỬA LỖI BUILD VÀ TỐI ƯU LUỒNG ĐĂNG KÝ (2026-05-23)

### Mục tiêu
1. Khắc phục lỗi build production trên Vercel liên quan đến `useSearchParams()`.
2. Đảm bảo 100% người dùng mới có mã giới thiệu (mặc định là 0).
3. Bổ sung tính năng đặt mật khẩu cho người dùng đăng ký qua Google.

### Các file đã sửa

#### 1. `app/complete-profile/page.tsx`
- **Fix Build**: Bọc toàn bộ logic trang vào `<Suspense>` để tránh lỗi SSG bailout khi sử dụng `useSearchParams()`.
- **Nâng cấp UI**: Thêm trường **Mật khẩu mới (Tùy chọn)** cho phép người dùng OAuth thiết lập mật khẩu để đăng nhập bằng ID/Email sau này.
- **Bảo toàn logic**: Giữ nguyên cơ chế tự động kiểm tra và chuyển hướng (redirect) nếu hồ sơ đã hoàn tất.

#### 2. `auth.ts`
- **Mandatory Referrer**: Cập nhật Custom Adapter để gán `referrerId: 0` ngay khi tạo User qua OAuth.
- **Genealogy Sync**: Luôn gọi `addUserToClosure` trong sự kiện `createUser` để đảm bảo mọi người dùng mới đều xuất hiện trong cây phả hệ, ngay cả khi không có link giới thiệu.

#### 3. `app/actions/auth-actions.ts`
- **`completeProfileAction`**:
  - Hỗ trợ nhận và mã hóa mật khẩu mới (bcrypt) cho người dùng Google.
  - Tự động gán `referrerId: 0` nếu giá trị này đang bị `null` (Safety Layer).
  - Tự động cập nhật cây phả hệ nếu gán mã giới thiệu mặc định.

#### 4. `GEMINI.md` (AGENTS.md)
- **Quy tắc mới**: Bổ sung cờ đỏ ❌ CẤM sử dụng lược bỏ `...` khi dùng lệnh `write`. Bắt buộc Agent phải đối chiếu nội dung file so với backup để bảo vệ code cũ của người dùng.

### Trạng thái
- ✅ Lỗi Build Production đã được xử lý.
- ✅ 100% người dùng mới đều có người giới thiệu (mặc định Admin #0).
- ✅ Người dùng Google có thể đặt mật khẩu fallback.
- ✅ Quy trình làm việc của AI Agent được thắt chặt để đảm bảo an toàn code.
- ✅ Build: `npx tsc --noEmit` — hoàn thành 0 lỗi.

---

## ✅ PHẦN 28: DI CHUYỂN VÀ BẢO MẬT TÍNH NĂNG XÓA NGƯỜI DÙNG (2026-05-23)

### Mục tiêu
Chuyển tính năng xóa người dùng hàng loạt từ `/tools/tca-sync` sang `/tools/students` để phù hợp với ngữ cảnh quản lý, đồng thời thắt chặt bảo mật chỉ cho phép ADMIN thực hiện.

### Các file đã sửa/tạo

#### 1. `app/actions/admin-actions.ts`
- **Thêm mới**: `bulkDeleteUsersAction` và `previewDeleteUsersAction`.
- **Bảo mật**: Bắt buộc gọi `checkAdmin()` để đảm bảo chỉ Admin tối cao mới có quyền xóa.
- **Logic**: Di chuyển toàn bộ logic xóa từ API Route sang Server Action để đồng bộ kiến trúc.

#### 2. `components/admin/students/DeleteByUserSection.tsx` [NEW]
- **Mô tả**: Component quản lý xóa người dùng hàng loạt.
- **Tính năng**: Hỗ trợ xóa theo dải ID (>=), xóa trong khoảng (Between) hoặc xóa theo danh sách ID tùy chọn.
- **An toàn**: Có chức năng "Xem trước" (Preview) để Admin kiểm tra số lượng bản ghi liên quan (Closures, Systems, TCA Members) trước khi xóa thật.

#### 3. `app/tools/students/page.tsx`
- **Tích hợp**: Đưa `DeleteByUserSection` vào đầu trang quản lý học viên.
- **Phân quyền**: Chỉ hiển thị bộ công cụ xóa nếu `session.user.role === Role.ADMIN`.

#### 4. `app/tools/tca-sync/page.tsx`
- **Dọn dẹp**: Gỡ bỏ component xóa cũ và logic liên quan.
- **Điều hướng**: Thêm thông báo hướng dẫn Admin chuyển sang trang Quản lý Thành viên để thực hiện các thao tác xóa.

#### 5. `app/api/sync-tca/delete-by-user/route.ts`
- **Bảo mật Layer 2**: Thêm kiểm tra `session.user.role === Role.ADMIN` cho API route cũ để ngăn chặn các truy cập trái phép qua HTTP request trực tiếp.

### Trạng thái
- ✅ Tính năng xóa người dùng đã được quy hoạch về đúng vị trí.
- ✅ Bảo mật tuyệt đối: Chỉ Admin mới thấy và dùng được tính năng này.
...
## ✅ PHẦN 35: REDESIGN NORMAL COURSE + isDailyChallenge PER-LESSON (2026-06-19)

### Mục tiêu
1. Thay đổi hành vi Course type `NORMAL`: tất cả lessons unlocked tự do (không tuần tự), popup nhắc nhở khi chuyển lesson mà chưa có bình luận
2. Thêm `isDailyChallenge` cho từng lesson: ghi đè ưu tiên — nếu `true` thì bắt buộc làm bài tập (ghi nhận) trước khi chuyển, nếu `false` thì theo logic của Course type
3. Cho phép admin cấu hình `isDailyChallenge` qua giao diện thêm/sửa bài học

### Các file đã sửa

#### `components/course/LessonSidebar.tsx`
- **Line 38**: Thêm `|| courseType === 'NORMAL'` vào unlock logic — NORMAL unlock tất cả lessons
- **Line 9**: Thêm `isDailyChallenge?: boolean` vào interface `Lesson`
- **Line 174**: Thêm badge `📝 Bài tập` cạnh title nếu `lesson.isDailyChallenge === true`

#### `components/course/CoursePlayer.tsx`
- **Line 567**: Thêm `courseType === 'NORMAL'` vào unlock logic của `LessonSidebarMobile`
- **Lines 22, 66-67**: Import `hasUserCommentedOnLesson`, thêm state `showCommentReminder`, `pendingLessonId`
- **Lines 141-161**: Thêm `isDailyChallenge` check TRƯỚC NORMAL comment check:
  - `isDailyChallenge = true` → hard block (mandatory assignment, no skip)
  - `isDailyChallenge = false` → NORMAL comment reminder (skip-able)
  - Comment reminder áp dụng cho mọi course type trừ LIB
- **Line 228**: Auto-advance gọi `handleLessonSelect(nextId, true)` — skip comment check
- **Lines 486-503**: Popup daily challenge "Bài <title> yêu cầu hoàn thành bài tập..."
- **Lines 582-586**: Badge `📝 Bài tập` trong `LessonSidebarMobile`

#### `app/actions/comment-actions.ts`
- **Lines 59-70**: Thêm server action `hasUserCommentedOnLesson(lessonId)` — kiểm tra user đã comment chưa
- **Lines 120-155**: Trong `createComment`, nếu course type là NORMAL → auto upsert `LessonProgress.status = 'COMPLETED'`

#### `components/admin/courses/AddLessonModal.tsx`
- Thêm state `isDailyChallenge` (default false)
- Thêm checkbox `📝 Bài tập bắt buộc` với mô tả
- Gửi `isDailyChallenge` trong POST body

#### `components/admin/courses/LessonEditModal.tsx`
- Thêm state `isDailyChallenge` (pre-populated từ `lesson.isDailyChallenge`)
- Thêm checkbox tương tự
- Gửi `isDailyChallenge` trong `onSave`

#### `app/api/courses/[id]/lessons/route.ts`
- Nhận `isDailyChallenge` từ request body
- Ghi vào `prisma.lesson.create` (`isDailyChallenge: isDailyChallenge ?? false`)

#### `app/actions/admin-actions.ts`
- `updateLessonAction`: Thêm `isDailyChallenge?: boolean` vào data type

### Trạng thái
- ✅ NORMAL course: tất cả lessons unlocked, không cần tuần tự
- ✅ NORMAL course: popup comment reminder khi chuyển lesson (skip-able)
- ✅ `isDailyChallenge = true` → hard block, bắt buộc làm bài tập
- ✅ `isDailyChallenge = false` → fallback theo Course type
- ✅ Comment reminder áp dụng cho mọi course type (CHALLENGE, NORMAL) trừ LIB
- ✅ Badge `📝 Bài tập` trên sidebar (desktop + mobile)
- ✅ Admin UI checkbox trong thêm/sửa bài học
- ✅ Backup: `plan_temp/isDailyChallenge_backup_20260619_1100.patch`
- ✅ Build: `npx tsc --noEmit` — 0 lỗi

---

## ✅ PHẦN 32: TRỢ LÝ TÀI KHOẢN — REGISTER OTP + VERIFY FORGOT OTP (2026-06-24)

### Mục tiêu
Hoàn thiện modal Trợ lý tài khoản: thêm UI nhập OTP cho flow đăng ký, sửa handleVerifyOtp gọi API xác thực trước khi cho phép nhập mật khẩu mới.

### Các file đã sửa

#### `components/auth/AccountAssistantModal.tsx`
- **Thêm `case 'register_otp':`** trong `renderCustomInput` — UI nhập OTP 6 số + masked email (giống forgot_otp)
- **Sửa `handleVerifyOtp`**: từ sync check length → async gọi API `/api/auth/verify-forgot-otp`, chỉ advance nếu OTP hợp lệ
- **Cập nhật `isLoadingAction`**: thêm `verify_otp`, `verify_register_otp`, `skip_register_otp`; xoá `register_password` (sync handler)
- **Thêm `registeredUserId`** vào deps của `handleAction` useCallback
- **Ẩn success banner** khi ở step `register_otp`

#### `app/api/auth/verify-forgot-otp/route.ts` [NEW]
- Endpoint kiểm tra OTP trong bảng `passwordReset` (theo userId + token + expiresAt)
- Không xoá token (chỉ verify) — token bị xoá khi reset-password hoàn tất
- Trả về `{ success: true }` nếu OTP hợp lệ, `{ error }` nếu sai/hết hạn

#### `prisma/seed-account-assistant.ts`
- Đã cập nhật seed với các step mới (`register_password`, `register_confirm`, `register_otp`)
- `found_account` set `isActive: false`

### Trạng thái
- ✅ UI OTP đăng ký (register_otp) hiển thị đúng
