...
async function handler(request: Request) {
  try {
    const authResult = isAuthorizedRequest(request as Request & { nextUrl?: { searchParams: URLSearchParams } }, { secretEnv: 'CRON_SECRET' })
    ...
    try {
      ...
      console.error('❌ brk-expiration: Failed to send Telegram error notification:', notifyErr);
    }
