...
export async function sendGmailFromSender(
  ...
  // Tạo nội dung email chuẩn MIME
  const utf8Subject = `=?utf-8?B?${Buffer.from(subject).toString('base64')}?=`;
  const utf8FromName = `=?utf-8?B?${Buffer.from("Học Viện BRK").toString('base64')}?=`;
  
  const messageParts = [
    `From: ${utf8FromName} <${sender.email}>`,
    `To: ${to}`,
  ...
  // Encode Base64url (theo yêu cầu của Gmail API)
...
export async function resolveRecipients(campaignId: number): Promise<Recipient[]> {
  ...
  if (campaign.recipientSource === "CSV") {
    // Giả định recipientCsvData là chuỗi JSON mảng [{email, name}]
