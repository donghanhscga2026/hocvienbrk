...
> **Mục đích**: Tài liệu này đặc tả luồng giao dịch tài chính, cơ chế phê duyệt thanh toán thủ công và tự động kích hoạt quyền truy cập khóa học cho thành viên.
...
Quy trình từ lúc thành viên nhấn "Mua khóa học" đến khi được vào học:
...
3. **Gửi minh chứng (Proof of Payment)**: Thành viên tải ảnh biên lai chuyển khoản (Proof Image) lên hệ thống. Ảnh được lưu trữ an toàn trên Supabase Storage.
4. **Phê duyệt (Verification)**: 
   ...
   - Khi nhấn "Xác nhận", Server Action `verifyPaymentAction` sẽ thực hiện đồng thời:
     ...
     - Gửi email thông báo kích hoạt thành công cho thành viên.
...
- Xem danh sách các yêu cầu đang chờ (PENDING). Kiểm tra khớp lệnh giữa ảnh biên lai thành viên gửi và biến động số dư ngân hàng của bạn.

**Bước 2: Phê duyệt/Từ chối**
- **Xác nhận**: Nếu thông tin đã chính xác, nhấn nút **"Xác nhận"**. Thành viên sẽ ngay lập tức vào học được.
- **Từ chối**: Nếu số tiền sai hoặc ảnh biên lai không hợp lệ, nhấn **"Từ chối"** và nhập lý do (vd: "Sai số tiền", "Nội dung chuyển khoản không khớp"). Thành viên sẽ nhận được thông báo để thực hiện lại.
...
- **Tôi lỡ tay từ chối một thanh toán đúng, phải làm sao?**
  - Bạn có thể chuyển sang Tab **"Tất cả"**, tìm lại đơn đó và thực hiện "Xác nhận" lại. Hệ thống sẽ cập nhật lại trạng thái cho thành viên.
- **Thành viên báo lỗi không tải được ảnh biên lai?**
  - Đảm bảo file ảnh của thành viên không quá lớn (dưới 5MB) và có định dạng phổ biến (JPG, PNG). Nếu vẫn lỗi, Admin có thể duyệt trực tiếp nếu đã nhận được tiền trong tài khoản.
