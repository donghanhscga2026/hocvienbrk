...
import { Eye, Edit, ExternalLink, Loader2, Plus, AlertCircle, CheckCircle, BarChart3 } from 'lucide-react'
import { getMySiteProfile } from '@/app/actions/site-profile-actions'

interface Profile {
  id: number; slug: string; title: string | null; isActive: boolean; isDefault: boolean; viewCount: number; heroImage: string | null; createdAt: Date | string; updatedAt: Date | string
}

export default function MySiteTab() {
  ...
  useEffect(() => {
    if (userId) { loadProfile() } else { setLoading(false) }
  }, [userId])
    ...
    return (
      <div className="p-4 max-w-lg mx-auto">
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center">
          ...
          <Link href="/login?redirect=/tools/my-site" className="inline-block px-6 py-2 bg-amber-500 text-white rounded-xl font-medium hover:bg-amber-600">Đăng nhập</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="p-4 max-w-lg mx-auto">
      ...
      {!profile ? (
        <div className="bg-white rounded-3xl border border-gray-100 p-6 text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">Tạo trang chủ riêng để giới thiệu khóa học và chia sẻ link affiliate.</p>
          <Link href="/tools/site-profiles/new" className="inline-block px-6 py-3 bg-orange-500 text-white rounded-xl font-medium hover:brightness-110 transition-all">Tạo trang chủ</Link>
        </div>
      ) : (
        <>
          <div className="bg-white rounded-3xl border border-gray-100 p-6 mb-4">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                {profile.isActive ? (
                  <div className="flex items-center gap-1 px-2 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold"><CheckCircle className="w-3 h-3" /> Đang hoạt động</div>
                ) : (
                  <div className="flex items-center gap-1 px-2 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold"><AlertCircle className="w-3 h-3" /> Chờ duyệt</div>
                )}
              </div>
              <div className="flex items-center gap-1 text-sm text-gray-500"><BarChart3 className="w-4 h-4" /><span>{profile.viewCount || 0} lượt xem</span></div>
            </div>
            <div className="space-y-3">
              <div className="flex justify-between items-center"><span className="text-sm text-gray-500">Tiêu đề</span><span className="font-medium text-gray-800">{profile.title || 'Chưa đặt'}</span></div>
              <div className="flex justify-between items-center"><span className="text-sm text-gray-500">URL</span><Link href={`/page/${profile.slug}`} className="font-mono text-sm text-orange-500 hover:underline" target="_blank">/page/{profile.slug}</Link></div>
            </div>
          </div>
            ...
            <div className="mt-4 bg-amber-50 border border-amber-200 rounded-2xl p-4">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-700 mt-1">Admin sẽ xem xét và kích hoạt trang trong thời gian sớm nhất.</p>
                </div>
              </div>
            </div>
          )}
