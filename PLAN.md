...
#### `lib/brk/commission-calculator.ts` & `scripts/replay/runner.ts`
- Vấn đề: Thiếu phần ghi nhận lịch sử biến động điểm tích lũy (MBP) và doanh số dồn (MBDT) cho ancestors khi có thành viên dưới hệ thống confirm chính thức.
- Fix: Tự động tạo giao dịch `BRKD_CREDIT` với số tiền `0 MBDT` vào ví MBDT (`BRKD`) của ancestors với description ghi nhận chi tiết số điểm và doanh số được cộng (Ví dụ: *"Cộng +19.042 MBP & dồn +14,281,263 MBDT từ F1 #1085 confirm chính thức"*).

### Các file đã tạo
#### `scripts/replay/apply-simulation-to-db.ts`
- Chức năng: Đọc simulation state cuối Ngày 14, dọn sạch transactions và promotions cũ của System 4, update số dư ví chuẩn và insert lại toàn bộ 837 transactions (bao gồm 269 transactions logs điểm) + 187 promotions mới của Simulation vào DB thật.
...
- ✅ Đã tích hợp ghi nhận lịch sử biến động điểm tích lũy (MBP) và doanh số dồn (MBDT) từ các F1/F2... confirm chính thức dưới dạng giao dịch ghi nhận đặc biệt (amount = 0) vào ví MBDT.
- ✅ Đã cập nhật và đồng bộ 100% code logic core của dự án, các APIs cron chạy thực tế và Simulation Replay.
- ✅ Đã thiết lập bảng database `BrkTimelineRecord` chuyên biệt lưu trữ sẵn lịch sử thăng tiến, chuyển Server Action `getMemberPromotionHistoryAction` sang SELECT trực tiếp siêu tốc.
- ✅ Đã sửa đổi UI Lịch sử thăng tiến của học viên trên trang gia phả (Genealogy) hiển thị đúng Grid 2 cột Trái/Phải (Doanh số/DS đối ứng ở cột trái, Thu nhập/Đối ứng ở cột phải) với các màu sắc Slate và Emerald sang trọng, đúng cỡ chữ, đơn vị và nhãn.
...
- ✅ Đã chạy script backfill thành công, Comparative Stats lệch bằng 0 tuyệt đối cho cả ví CASH, ví MBDT, Cấp bậc và Điểm số của toàn bộ 84 học viên.
- ✅ Dự án build thành công 100% (`npx tsc --noEmit`).

## ✅ Sửa lỗi dồn Thu nhập ví BRKD, sắp xếp sự kiện Timeline & Tích hợp vào Services chạy thật (2026-07-16)

### Mục tiêu
- Khắc phục lỗi dồn doanh số MBDT của F1/F2 lên ví thu nhập BRKD của upline.
- Tối ưu hóa thứ tự hiển thị các sự kiện xảy ra trong cùng giây: ưu tiên `TRANSACTION` cộng điểm/doanh số hiển thị trước sự kiện thăng cấp `LEVEL_UP`.
- Loại bỏ các points thăng cấp trùng lặp khỏi TRANSACTION để tránh in trùng lặp sự kiện thăng cấp trên Timeline.
- Tích hợp ghi nhận timeline (`createBrkTimelineRecord`) vào 4 service cốt lõi chạy thật của hệ thống.

### Các file đã sửa
#### `lib/brk/activation-service.ts`
- Thêm timeline logging cho các sự kiện học viên kích hoạt (`ACTIVATION`), học viên mới đăng ký (`TRANSACTION` F1_ACTIVE/F2_ACTIVE), và hoàn phí khi hết cân nhắc (`RETURN_FEE`).

#### `lib/brk/commission-calculator.ts`
- Thêm timeline logging khi phát sinh commission chênh lệch thực tế hoặc biến động points/doanh số dồn (`amountBrkd = 0` ở các mốc points dồn chênh lệch % = 0).

#### `lib/brk/level-manager.ts`
- Thêm timeline logging cho sự kiện thăng cấp thực tế (`LEVEL_UP` từ Cấp 2 trở lên) và các mốc thưởng voucher thăng cấp hoặc 2F1 (`VOUCHER_CREDIT`).

#### `lib/brk/revenue-share-service.ts`
- Thêm timeline logging cho phần chia thưởng đồng chia (`REVENUE_SHARE`).

#### `scripts/replay/runner.ts`
- Lag mốc thời gian của commission và thăng cấp lùi lại 1-2 giây so với confirm để phản ánh chính xác độ trễ thực tế.
...
### Trạng thái
- ✅ Đã sửa đổi code và chạy simulation tạo state mới thành công.
- ✅ Khôi phục dữ liệu DB backfill thành công, đối soát khớp lệch 0 tuyệt đối.
- ✅ Ví thu nhập MBDT của upline giữ nguyên không bị cộng khống doanh số F1.
- ✅ Dòng thăng cấp hiển thị đúng điểm số đã cộng (ví dụ Cấp 2 hiển thị 82.05 MBP, thỏa mãn điều kiện > 50 MBP).
...
- ✅ Tinh chỉnh Lịch sử thăng tiến modal:
  ...
  - Loại bỏ hoàn toàn footer div chứa nút "ĐÓNG" ở cuối Modal vì đã có nút (X) góc trên bên phải.
  - Đưa nút Sắp xếp lên Header Modal: Thay thế bộ chọn combox list cũ bằng nút bấm icon (mũi tên xuống `ArrowDown` - mặc định cho Mới nhất trước, khi click chuyển thành mũi tên lên `ArrowUp` - cho Cũ nhất trước) và loại bỏ hoàn toàn thanh Toolbar trống ở body.
- ✅ Mở rộng ghi nhận đăng ký F3: Nâng cấp ghi nhận log và timeline sự kiện "Học viên mới đăng ký" lên 3 cấp bảo trợ gần nhất (F1, F2 và F3) tại code live `activation-service.ts`.
- ✅ Bổ sung tên học viên vào log confirm chính thức: Cập nhật format chuỗi mô tả tích lũy `"Cộng +... MBP & Doanh số +... MBDT từ F... #[ID] [Tên] đã chính thức tham gia"` để chứa cả ID và Tên thành viên mới tại code live `commission-calculator.ts`.
- ✅ Nhất quán hóa logic Replay và Rebuild: Refactor file `lib/brk/rebuild-service.ts`, xóa bỏ các logic và hàm tự tính toán trùng lặp (như `confirmAndLevelUp`, `confirmMember`), thay thế hoàn toàn bằng việc gọi trực tiếp các hàm nghiệp vụ live chính thức của hệ thống (`activateBrkMember`, `processGracePeriodExpirations`). Mở rộng `processGracePeriodExpirations` nhận tham số `now: Date` để hỗ trợ dòng thời gian giả lập.
- ✅ Dọn sạch và tích hợp Replay: Xóa bỏ 2 file giả lập cũ `runner.ts` và `apply-simulation-to-db.ts`. Cập nhật script `run_all_days.ts` gọi trực tiếp `rebuildSystem4Data('B')` của code live để đảm bảo đồng bộ 100% khi chạy Replay.
...
## ✅ Khôi phục Gmail Watch sau CRON_SECRET fix (2026-07-24)

### Vấn đề
- Cron `gmail-watch` (daily midnight UTC) fail do sai `CRON_SECRET` (file `.env.local` có quotes + `\r`).
- Gmail Watch hết hạn sau 7 ngày → push notification từ Google Pub/Sub ngừng hoạt động.
- Email thanh toán của học viên không được auto-verify real-time.
- Hậu quả: 27 enrollment `PENDING` chờ xử lý.

### Đã làm
#### Kiểm tra cấu hình
- `.env` và `.env.local` đã có `CRON_SECRET=BrkAcademySecret123` đúng, không quotes, không `\r`.

#### Trigger manual gmail-watch
- Gọi `GET /api/cron/gmail-watch` với `Authorization: Bearer BrkAcademySecret123`.
- Kết quả: ✅ Watch renewed (`historyId: 60962`, expiration ~7 ngày).
- Scan: 2 email trong 2 ngày gần, không khớp pending nào.
- Push notification sẽ hoạt động trở lại cho các thanh toán sau.

### Các file đã kiểm tra
#### `app/api/cron/gmail-watch/route.ts`
- CRON_SECRET check dùng `.trim()` → an toàn với whitespace.
- Lệnh renew watch + scan email hoạt động bình thường.

#### `lib/auto-verify.ts`
- `processPaymentEmails()` kiểm tra rate-limit penalty + scan lock trước khi chạy.
- Scan 2 ngày gần nhất, parse email Sacombank, match với pending enrollment.

#### `app/api/webhooks/gmail/route.ts`
- Endpoint POST không auth, nhận push từ Google Pub/Sub → gọi `processPaymentEmails()`.

### Trạng thái
- ✅ Gmail Watch đã được renew, push notification hoạt động.
...
### Trạng thái
- ✅ `npx tsc --noEmit` — Exit code 0
- ✅ Code runtime sạch chuỗi "Học Viện/Học viên" và BRK hiển thị (grep xác nhận)
