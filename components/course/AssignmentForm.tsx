...
interface AssignmentFormProps {
    ...
    lessonOrder: number
    startedAt: Date | null
    videoPercent: number // 0-100 từ VideoPlayer
        ...
        <div className="flex flex-col h-full bg-[#FFFDE7] overflow-y-auto">
            ...
            <div className="sticky top-0 z-10 bg-[#FFFDE7] border-b border-orange-200 px-4 py-3">
                <div className="flex items-center justify-between mb-1">
                    ...
                    >
                        <Info className="w-3 h-3" /> Quy tắc
                    </button>
                </div>
                <p className="text-xs text-gray-500">
                    Ngày cần hoàn thành:
                    <span className="font-semibold text-gray-700 ml-1">{formatDate(deadline)}</span>
                </p>

                {/* Tổng điểm */}
                <div className="mt-3 bg-[#FFF3CD] border border-orange-200 rounded-xl px-4 py-3 flex items-center justify-between">
                    <span className="text-xs font-bold uppercase tracking-wide text-gray-600">Tổng điểm tạm tính</span>
                    <span className="text-2xl font-black text-orange-500">{total}/10</span>
                </div>
                ...
                <button
                    ...
                    disabled={loading}
                    className="mt-3 w-full flex items-center justify-center gap-2 bg-orange-500 hover:bg-orange-600 active:scale-[0.98] text-white font-black rounded-xl py-3 transition-all shadow-md disabled:opacity-60"
                >
                    {loading
            ...
            <div className="flex flex-col gap-3 p-4">
                ...
                <div className="bg-white rounded-xl border border-gray-200 px-4 py-3 shadow-sm">
                    ...
                    <div className="mt-2">
                        ...
                        <p className="text-xs text-gray-400 mt-1">Đang xem: {videoPercent.toFixed(0)}%</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 px-4 py-3 shadow-sm">
                    ...
                    <textarea
                        ...
                        rows={4}
                        className="w-full mt-1 text-sm text-gray-800 border border-gray-200 rounded-lg p-2.5 resize-none focus:outline-none focus:ring-2 focus:ring-orange-300 placeholder:text-gray-300"
                    />
                    <p className="text-[10px] text-gray-400 mt-1">{reflection.length} ký tự {reflection.length >= 50 ? '✓ Sâu sắc' : '(cần ≥ 50 để đạt max)'}</p>
                </div>
                    ...
                    <div className="flex flex-col gap-2 mt-1">
                        {links.map((link, i) => (
                            <input
                                ...
                                className="w-full text-sm text-gray-800 border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-300 placeholder:text-gray-300"
                            />
                        ))}
                    </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 px-4 py-3 shadow-sm">
                    ...
                    <div className="mt-2 space-y-1.5 text-sm">
                        <div className="flex justify-between">
                            ...
                            <span className="text-green-600 font-bold">+1 điểm</span>
                        </div>
                        <div className="flex justify-between">
                            <span className="text-gray-500">Muộn (Sau 23:59):</span>
                            <span className="text-red-500 font-bold">-1 điểm</span>
                        </div>
                    </div>
                    <p className="text-[10px] text-gray-400 mt-2">* Hệ thống tự động ghi nhận theo thời gian thực.</p>
                </div>
